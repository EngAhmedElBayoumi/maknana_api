...
class MachineCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = machine
        exclude = ['factory']  # Exclude factory field from validation
        extra_kwargs = {
...
class Factory2Serializer(serializers.ModelSerializer):
    machines = serializers.ListField(child=MachineCreateSerializer(), required=False, allow_empty=True)

    def __init__(self, *args, exclude_user=False, **kwargs):
    ...
    class Meta:
        model = factory
        fields = ['name', 'location', 'phone', 'country_code', 'mobile_code', 'responsible_user', 'machines', 'user']
        extra_kwargs = {
            'user': {'required': False}
        }
