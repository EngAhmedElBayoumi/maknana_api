...
from .serializers import (
    ...
    ResetPasswordSerializer, SetNewPasswordSerializer, UserSerializer , CustomerWithFactoriesSerializer
)
...
class AccountViewSet(ModelViewSet):
    ...
    # action to create customer with factories and machines
    # action to create customer with factories and machines
    @action(detail=False, methods=['post'], url_path='create_customer', permission_classes=[IsAuthenticated])
    @swagger_auto_schema(
        operation_description="Create a new customer with associated factories and machines.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'name': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's full name", max_length=255),
                'email': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's email address", format='email'),
                'first_phone': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's primary phone number", max_length=15),
                'password': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's password", write_only=True),
                'second_phone': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's secondary phone number (optional)", max_length=15, nullable=True),
                'location': openapi.Schema(type=openapi.TYPE_STRING, description="Customer's location (optional)", max_length=255, nullable=True),
                'factories': openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    description="List of factories associated with the customer",
                    items=openapi.Schema(
                        type=openapi.TYPE_OBJECT,
                        properties={
                            'name': openapi.Schema(type=openapi.TYPE_STRING, description="Factory name", max_length=100),
                            'location': openapi.Schema(type=openapi.TYPE_STRING, description="Factory location", max_length=100),
                            'phone': openapi.Schema(type=openapi.TYPE_STRING, description="Factory phone number", max_length=100),
                            'country_code': openapi.Schema(type=openapi.TYPE_STRING, description="Country code for the phone number", max_length=100),
                            'mobile_code': openapi.Schema(type=openapi.TYPE_STRING, description="Mobile code for the phone number", max_length=100),
                            'responsible_user': openapi.Schema(type=openapi.TYPE_STRING, description="Name of the responsible user for the factory", max_length=100),
                            'machines': openapi.Schema(
                                type=openapi.TYPE_ARRAY,
                                description="List of machines in the factory",
                                items=openapi.Schema(
                                    type=openapi.TYPE_OBJECT,
                                    properties={
                                        'name': openapi.Schema(type=openapi.TYPE_STRING, description="Machine name", max_length=100),
                                        'status': openapi.Schema(type=openapi.TYPE_STRING, description="Machine status (e.g., Operational, Under Maintenance)", max_length=100),
                                        'warranty_status': openapi.Schema(type=openapi.TYPE_STRING, description="Warranty status of the machine", enum=['warranty', 'not warranty']),
                                        'last_maintenance': openapi.Schema(type=openapi.TYPE_STRING, description="Date of last maintenance (YYYY-MM-DD)", format='date'),
                                        'machine_code': openapi.Schema(type=openapi.TYPE_STRING, description="Unique code for the machine (optional)", max_length=100, nullable=True),
                                    },
                                    required=['name', 'status', 'warranty_status', 'last_maintenance']
                                )
                            )
                        },
                        required=['name', 'location', 'phone', 'country_code', 'mobile_code', 'responsible_user']
                    )
                )
            },
            required=['name', 'email', 'first_phone', 'password', 'factories']
        ),
        responses={
            201: openapi.Response(description="Customer created successfully.", schema=UserSerializer),
            400: openapi.Response(description="Invalid data provided.")
        }
    )
    def create_customer(self, request):
        serializer = CustomerWithFactoriesSerializer(data=request.data)
        if serializer.is_valid():
            user = serializer.save()
